...
\BLOCK{ for path in paths }
    ...
    \pathcard[%
        ...
        \BLOCK{ endif }
        \BLOCK{ if path.big_passive == true }
        big passive,
        \BLOCK{ endif }
    ]{%
    \BLOCK{ if path.name != "Heirloom" }
        \BLOCK{ if path.cards|selectattr("linked", "eq", None)|list|length > 0 and path.cards|selectattr("linked", "eq", None)|list|last|attr("linked_to")|length > 0 }
            \addtolength{\yoffset}{1.5mm}%
        \BLOCK{ endif }
        \BLOCK{ for card in path.cards|reverse }
            \pathrow[%
                \BLOCK{ if card.path_card_name != None }
                name={\VAR{card.path_card_name}},
                \BLOCK{ else }
                name={\VAR{card.name}},
                \BLOCK{ endif }
